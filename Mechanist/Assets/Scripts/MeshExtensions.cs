...
namespace ProceduralPrimitives
{
    ...
    public static class MeshExtensions
    {
            ...
            int radialSegments, int heightSegments)
        {
            ...
            int numVerts = vertCols * vertRows + 2 * vertCols + 2;

            int numSideTris = radialSegments * heightSegments * 2;
            int numCapTris = radialSegments * 2;

            // each triangle requires three indices
            int trisArrayLength = (numSideTris + numCapTris) * 3;
            ...
            for (int row = 0; row < vertRows; row++)
            {
                for (int col = 0; col < vertCols; col++)
                {
                    ...
                    float ratio = (float)row / (vertRows - 1);
                    float radius = Mathf.Lerp(bottomRadius, topRadius, ratio);
                    ...
                    int vertIdx = row * vertCols + col;
                    vertices[vertIdx] = new Vector3(
                    ...
                    int triIdx = ((row - 1) * radialSegments * 6 + col * 6);
                    triangles[triIdx + 0] = row * vertCols + col;
                    triangles[triIdx + 1] = row * vertCols + col + 1;
                    triangles[triIdx + 2] = (row - 1) * vertCols + col;

                    triangles[triIdx + 3] = (row - 1) * vertCols + col;
                    triangles[triIdx + 4] = row * vertCols + col + 1;
                    triangles[triIdx + 5] = (row - 1) * vertCols + col + 1;
                }
            }

            // Draw caps
            vertices[numVerts - 2] = new Vector3(0, height / 2, 0); // top center
            vertices[numVerts - 1] = new Vector3(0, -height / 2, 0); // bottom center
            uvs[numVerts - 2] = new Vector2(0, 1 + topRadius);
            uvs[numVerts - 1] = new Vector2(0, -bottomRadius);

            // perimeter vertices of the cap
            int topPerimeterVertIdxOffset = vertCols * vertRows;
            int bottomPerimeterVertIdxOffset = vertCols * vertRows + vertCols;
            for (int i = 0; i <= radialSegments; i++)
            {
                float angle = i * angleStep;
                vertices[topPerimeterVertIdxOffset + i] = new Vector3(
                    topRadius * Mathf.Cos(angle),
                    height / 2,
                    topRadius * Mathf.Sin(angle)
                );
                vertices[bottomPerimeterVertIdxOffset + i] = new Vector3(
                    bottomRadius * Mathf.Cos(angle),
                    -height / 2,
                    bottomRadius * Mathf.Sin(angle)
                );
                uvs[topPerimeterVertIdxOffset + i] = new Vector2(
                    topRadius * Mathf.Cos(angle), 1 + topRadius + topRadius * Mathf.Sin(angle)
                );
                uvs[bottomPerimeterVertIdxOffset + i] = new Vector2(
                    bottomRadius * Mathf.Cos(angle), -bottomRadius + bottomRadius * Mathf.Sin(angle)
                );
            }

            // cap triangles
            int topTriIdxOffset = numSideTris * 3;
            int bottomTriIdxOffset = (numSideTris + radialSegments) * 3;
            for (int i = 0; i < radialSegments; i++)
            {
                // top
                triangles[topTriIdxOffset + 0] = numVerts - 2;
                triangles[topTriIdxOffset + 1] = topPerimeterVertIdxOffset + i + 1;
                triangles[topTriIdxOffset + 2] = topPerimeterVertIdxOffset + i;

                // bottom
                triangles[bottomTriIdxOffset + 0] = numVerts - 1;
                triangles[bottomTriIdxOffset + 1] = bottomPerimeterVertIdxOffset + i;
                triangles[bottomTriIdxOffset + 2] = bottomPerimeterVertIdxOffset + i + 1;
            ...
            for (int row = 0; row < vertRows; row++)
            {
                ...
                int i1 = row * vertCols + 0;
                int i2 = row * vertCols + vertCols - 1;
