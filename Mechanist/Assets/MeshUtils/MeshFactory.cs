...
namespace MeshUtils
{
    public static class MeshFactory
    {
        ...
        public static Mesh CreateCone(float p_height, float p_bottomRadius, float p_topRadius, int p_sideCount,
        ...
        public static Mesh CreateTube(float p_height, int p_sideCount, float p_bottomRadius, float p_bottomThickness,
        ...
        public static Mesh CreateSphere(float p_radius, int p_longitudeCount, int p_lattitudeCount)
        {
            ...
            for (int lat = 0; lat < p_lattitudeCount; lat++)
            {
                ...
                for (int lon = 0; lon <= p_longitudeCount; lon++)
                {
                    ...
                    vertices[lon + lat * (p_longitudeCount + 1) + 1] =
                        new Vector3(sin1 * cos2, cos1, sin1 * sin2) * p_radius;
                }
            }
            ...
            for (int lat = 0; lat < p_lattitudeCount; lat++)
            for (int lon = 0; lon <= p_longitudeCount; lon++)
                uvs[lon + lat * (p_longitudeCount + 1) + 1] = new Vector2((float)lon / p_longitudeCount,
                    1f - (float)(lat + 1) / (p_lattitudeCount + 1));
