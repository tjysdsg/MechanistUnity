...
namespace ScriptableObjectArchitecture.Editor
{
    ...
    public sealed class BaseReferenceDrawer : PropertyDrawer
    {
        ...
        {
            get { return BaseReferenceHelper.GetValueType(fieldInfo); }
        }

        private bool SupportsMultiLine
        {
            get { return SOArchitecture_EditorUtility.SupportsMultiLine(ValueType); }
        }
        ...
        {
            return new Rect(position.x, valueRect.y + EditorGUIUtility.singleLineHeight, position.width,
                GenericPropertyDrawer.GetHeight(constantValue, ValueType));
        }
        ...
        private Rect GetMultiLineFieldRect(Rect position)
        {
            return EditorGUI.IndentedRect(new Rect
            {
                ...
                size = new Vector2(position.width,
                    EditorGUI.GetPropertyHeight(constantValue) + EditorGUIUtility.singleLineHeight)
            });
        }

        private bool ShouldDrawMultiLineField()
        {
            return useConstant.boolValue && SupportsMultiLine &&
                   EditorGUI.GetPropertyHeight(constantValue) > EditorGUIUtility.singleLineHeight;
        }
