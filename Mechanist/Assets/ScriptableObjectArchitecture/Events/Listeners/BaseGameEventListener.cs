...
{
    public abstract class BaseGameEventListener<TType, TEvent, TResponse> : DebuggableGameEventListener,
        IGameEventListener<TType>
        ...
        where TResponse : UnityEvent<TType>
    {
        ...
        [SerializeField] private TEvent _previouslyRegisteredEvent = default(TEvent);
        [SerializeField] private TEvent _event = default(TEvent);
        [SerializeField] private TResponse _response = default(TResponse);

        [SerializeField] protected TType _debugValue = default(TType);
...
#pragma warning disable 0414
        [SerializeField] private bool _showDebugFields = false;
...
#pragma warning restore
        ...
        protected void CreateDebugEntry(UnityEventBase response)
        {
            ...
            for (int i = 0; i < response.GetPersistentEventCount(); i++)
            {
                ...
                if (Vector3.Distance(gameObject.transform.position, gameObjectTarget.transform.position) <=
                    EVENT_MIN_DISTANCE)
                    continue;
        ...
        private void DrawEvent(int index)
        {
            ...
            if (debugEvent.Offset >=
                Vector3.Distance(gameObject.transform.position, debugEvent.Target.transform.position))
            {
