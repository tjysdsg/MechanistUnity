...
namespace RuntimeTransformHandle
{
    public class ScaleHandle : MonoBehaviour
    {
        ...
        public ScaleHandle Initialize(RuntimeTransformHandle p_parentTransformHandle)
        {
            ...
            if (_parentTransformHandle.axes != HandleAxes.X && _parentTransformHandle.axes != HandleAxes.Y &&
                _parentTransformHandle.axes != HandleAxes.Z)
            {
                _globalAxis = new GameObject().AddComponent<ScaleGlobal>()
                    .Initialize(_parentTransformHandle, HandleBase.GetVectorFromAxes(_parentTransformHandle.axes),
                        Color.white);
